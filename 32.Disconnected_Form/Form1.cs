...
namespace _32.Disconnected_Form
{
    public partial class Form1 : Form
    {
        ...
        {
            SqlDataAdapter adp = new SqlDataAdapter("select * from urunler", baglanti); //data readerdan farkı: açma kapama yapmıyoruz. select komutu için geçerli
        ...
        {
            SqlDataAdapter adp = new SqlDataAdapter("select * from Urunler where KategoriID=2", baglanti);
            DataTable dt = new DataTable();
            adp.Fill(dt);
            dgvListe.DataSource = dt;

            lblUrunBaslik.Text = gıdaToolStripMenuItem.Text;

        }

        private void temizlikToolStripMenuItem_Click(object sender, EventArgs e)
        {
            SqlDataAdapter adp = new SqlDataAdapter("select * from Urunler where KategoriID=5", baglanti);
            DataTable dt = new DataTable();
            adp.Fill(dt);
            dgvListe.DataSource = dt;

            lblUrunBaslik.Text = temizlikToolStripMenuItem.Text;
        }
        ...
        private void silToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                int id = Convert.ToInt32(dgvListe.CurrentRow.Cells["UrunID"].Value);
                SqlCommand komut = new SqlCommand("delete Urunler where UrunID=" + id, baglanti);
                baglanti.Open();
                try
                {
                    int etk = komut.ExecuteNonQuery();
